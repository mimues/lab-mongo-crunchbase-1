      ...
      rl.question('Type an option: ', (option) => {
        switch (option) {
          ...
          case "5":
            ...
            db.collection('companies').find({ $and: [{ founded_year: 2004 }, {$or: [{ founded_month: 4 }, { founded_month: 5 }, { founded_month: 6 }]}] }, { name: 1, _id: 0 }).sort({ founded_month: 1, founded_day: 1}).toArray((error, result) => {
              if (error) {
                console.log(error);
                rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
              } else {
          ...
          db.collection('companies').find({}, {name: 1, number_of_employees:1, _id: 0}).sort({number_of_employees: 1}).skip(18790).limit(10).toArray((error, result) => {
            if (error) {
              console.log(error);
              rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
            } else {
          ...
          db.collection('companies').find({name: 'Facebook'}, {name: 1, "relationships.person.permalink":1, _id: 0}).toArray((error, result) => {
            if (error) {
              console.log(error);
              rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
            } else {
          ...
          db.collection('companies').find({name: 'Facebook'}, {name: 1, 'competitions.competitor.name':1, _id: 0}).toArray((error, result) => {
            if (error) {
              console.log(error);
              rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
            } else {
          ...
          db.collection('companies').find({ $and: [{ tag_list: { $regex:"social-networking"} }, {founded_year: { $gte: 2002 } }, {founded_year: { $lte: 2016 } }] }, {name: 1, _id: 0}).count((error, result) => {
            if (error) {
              console.log(error);
              rl.question(`\nType enter to continue: `, (answer) => { mainMenu() });
            } else {
